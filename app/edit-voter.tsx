...
export default function EditVoterScreen() {
  ...
  const { updateVoter, leaders, voters } = useData();
  ...
  // Carregar dados do eleitor
  useEffect(() => {
    const voter = voters.find((v) => v.id === id);
    if (voter) {
      // Converter data ISO para formato DD/MM/YYYY
      const [year, month, day] = voter.birthDate.split(/[-T]/);
      const formattedDate = `${day}/${month}/${year}`;

      setFormData({
        name: voter.name,
        cpf: formatCPF(voter.cpf || ""),
        voterRegistration: formatVoterRegistration(voter.voterRegistration || ""),
        birthDate: formattedDate,
        phone: formatPhone(voter.phone),
        // Campos de endereço
        cep: formatCep(voter.cep || ""),
        street: voter.street || "",
        number: voter.number || "",
        ...
        state: voter.state || "",
        // Outros
        votesCount: voter.votesCount.toString(),
        leaderId: voter.leaderId,
        leaderName: voter.leaderName || "",
        notes: voter.notes || "",
      });

      // Configurar data selecionada para o date picker
      setSelectedDate(new Date(parseInt(year), parseInt(month) - 1, parseInt(day)));
    } else {
      showToast({ type: 'error', title: 'Erro', message: 'Eleitor não encontrado' });
      router.back();
    }
  }, [id, voters]);
  ...
  const validateForm = () => {
    ...
    if (!formData.birthDate.trim()) {
      showToast({ type: 'error', title: 'Campo obrigatório', message: 'Data de nascimento é obrigatória' });
      return false;
    }
    if (formData.birthDate.replace(/\D/g, "").length !== 8) {
    ...
    if (!formData.votesCount.trim()) {
      showToast({ type: 'error', title: 'Campo obrigatório', message: 'Quantidade de votos é obrigatória' });
      return false;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await updateVoter(id as string, {
      ...
      showToast({ type: 'error', title: 'Erro', message: error.message || 'Não foi possível atualizar o eleitor' });
    } finally {
      setLoading(false);
    }
  };
