...
export default function AddProjectScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (status === "em_tramitacao" || status === "aprovado" || status === "rejeitado") {
        timeline.push({
          id: (Date.now() + 1).toString(),
          date: new Date().toISOString(),
          description: "Projeto em tramitação",
          type: "tramitacao",
        });
      }
      ...
      showToast({ type: 'success', title: 'Sucesso', message: 'Projeto cadastrado com sucesso!' });
      setTimeout(() => router.back(), 1500);
    } catch (error) {
      console.error("Error adding project:", error);
      showToast({ type: 'error', title: 'Erro', message: 'Não foi possível cadastrar o projeto' });
    } finally {
      setIsLoading(false);
    }
  };
