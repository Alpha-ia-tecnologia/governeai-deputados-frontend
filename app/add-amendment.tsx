...
export default function AddAmendmentScreen() {
  const router = useRouter();
  const { addAmendment } = useData();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      showToast({ type: 'success', title: 'Sucesso', message: 'Emenda cadastrada com sucesso!' });
      setTimeout(() => router.back(), 1500);
    } catch (error) {
      console.error("Error adding amendment:", error);
      showToast({ type: 'error', title: 'Erro', message: 'Não foi possível cadastrar a emenda' });
    } finally {
      setIsLoading(false);
    }
  };
